...
\usepackage[american]{babel} % this package sets the standard conventions for things like hyphenation, date formats, etc.
\usepackage[utf8]{inputenc} % Not sure about Unicode and character sets? Try this: https://www.joelonsoftware.com/2003/10/08/the-absolute-minimum-every-software-developer-absolutely-positively-must-know-about-unicode-and-character-sets-no-excuses/
\usepackage{graphicx} % this package allows us to add figures and graphics to the document
\usepackage{color} %this package allows us to generate colors for fonts and the like
\usepackage{dirtytalk}  % this package has only one function - it puts things in quotes with the \say{} command
\graphicspath{ {img/} }  % this command sets the file path for the storage of image files used in the graphicx package
